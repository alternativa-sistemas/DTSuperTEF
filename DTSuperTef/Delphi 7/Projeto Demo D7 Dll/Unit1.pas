...
uses
 ...
 procedure Pagamento_Detalha(authorization_sh: WideString;
  ...
  procedure Pagamento_transaction_type(Result: PAnsiChar); stdcall; external 'supertef.dll' name 'Pagamento_transaction_type';
  procedure Pagamento_installment_count(Result: PAnsiChar); stdcall; external 'supertef.dll' name 'Pagamento_installment_count';
  procedure Pagamento_installment_type(Result: PAnsiChar); stdcall; external 'supertef.dll' name 'Pagamento_installment_type';
...
begin
  authorization_sh := EditTokenSH.Text;
  chave_cliente := EditChaveCliente.Text;
...
pResult : array [0..255] of AnsiChar;
resultado: string;
authorization_sh,chave_cliente,pos_id : string;

begin
 authorization_sh := EditTokenSH.Text;
  chave_cliente := EditChaveCliente.Text;
    pos_id :=     EditIDPOS.Text;


    Pagamento_Cadastro(authorization_sh,
    chave_cliente,
    strtoint(pos_id),0,1,1,
    '10,55',
    '123',
    'teste',true,pResult);
...
Pagamento_Detalha(authorization_sh,strtoint(Edit12.text),pResult);
  ...
  memo7.Lines.Add('acquirer_cnpj: '+pResult);
   ...
   pResult := '';
  Pagamento_transaction_type(pResult);
  memo7.Lines.Add('transaction_type: '+pResult);

   pResult := '';
  Pagamento_installment_count(pResult);
  memo7.Lines.Add('installment_count: '+pResult);

   pResult := '';
  Pagamento_installment_type(pResult);
  memo7.Lines.Add('installment_type: '+pResult);
