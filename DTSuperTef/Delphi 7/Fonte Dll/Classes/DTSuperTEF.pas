...
type
  ...
  private
    ...
    procedure PrepareRequest(const AResource: string;
  ...
  public
    ...
    // === Clientes ===
    function CriarCliente(Ativo: Integer; const CNPJ_CPF, NomeEmpresa,
      Contato: string; LimitePOS: Integer; const SitefEmpresa, SitefCNPJ_CPF,
      SitefBanco: string): TJSONObject;

    function ListarClientes: TJSONObject;

    function DetalharCliente(const Chave: string): TJSONObject;

    function AtualizarCliente(const Chave: string; Ativo: Integer;
      const CNPJ_CPF, NomeEmpresa, Contato: string; LimitePOS: Integer;
      const SitefEmpresa, SitefCNPJ_CPF, SitefBanco: string): TJSONObject;

    // === POS ===
    function CriarPOS(const ClienteChave: string; Status: Integer;
      const Nome: string): TJSONObject;

    function ListarPOS(const ClienteChave: string): TJSONObject;

    function DetalharPOS(APOSId: Integer): TJSONObject;

    function AtualizarPOS(APOSId: Integer; const ClienteChave: string;
      Status: Integer; const Nome: string): TJSONObject;

    function ExcluirPOS(APOSId: Integer; const ClienteChave: string)
      : TJSONObject;

    // === Pagamentos ===
    function CriarPagamento(const ClienteChave: string; POSId: Integer;
      const TransactionType: string; InstallmentCount, InstallmentType: Integer;
      Amount: Currency; const OrderID, Description: string;
      print_receipt: Boolean): TJSONObject;
...
procedure TDTSuperTEF.PrepareRequest(const AResource: string;
  AMethod: TRESTRequestMethod);
begin
  ...
  FRequest.AddParameter('Authorization', 'Bearer ' + FToken, pkHTTPHEADER,
    [poDoNotEncode]);
  FRequest.Body.ClearBody;
  FRequest.ConnectTimeout := 20000;
  FRequest.ReadTimeout := 20000;
end;
...
function TDTSuperTEF.CriarCliente(Ativo: Integer;
  const CNPJ_CPF, NomeEmpresa, Contato: string; LimitePOS: Integer;
  const SitefEmpresa, SitefCNPJ_CPF, SitefBanco: string): TJSONObject;
var
  JSON: TJSONObject;
...
function TDTSuperTEF.AtualizarCliente(const Chave: string; Ativo: Integer;
  const CNPJ_CPF, NomeEmpresa, Contato: string; LimitePOS: Integer;
  const SitefEmpresa, SitefCNPJ_CPF, SitefBanco: string): TJSONObject;
var
  JSON: TJSONObject;
...
function TDTSuperTEF.CriarPOS(const ClienteChave: string; Status: Integer;
  const Nome: string): TJSONObject;
var
  JSON: TJSONObject;
begin
...
function TDTSuperTEF.AtualizarPOS(APOSId: Integer; const ClienteChave: string;
  Status: Integer; const Nome: string): TJSONObject;
var
  JSON: TJSONObject;
begin
...
function TDTSuperTEF.ExcluirPOS(APOSId: Integer; const ClienteChave: string)
  : TJSONObject;
var
  JSON: TJSONObject;
begin
...
function TDTSuperTEF.CriarPagamento(const ClienteChave: string; POSId: Integer;
  const TransactionType: string; InstallmentCount, InstallmentType: Integer;
  Amount: Currency; const OrderID, Description: string; print_receipt: Boolean)
  : TJSONObject;
var
  JSON: TJSONObject;
begin
  ...
  try
    ...
    JSON.AddPair('amount', TJSONNumber.Create(StringReplace(FloatToStrF(Amount,
      ffFixed, 15, 2), ',', '.', [rfReplaceAll])));
    JSON.AddPair('order_id', OrderID);
    JSON.AddPair('description', Description);
    JSON.AddPair('print_receipt', TJSONBool.Create(print_receipt));
